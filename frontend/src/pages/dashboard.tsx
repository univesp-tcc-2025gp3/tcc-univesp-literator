...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    handleListFavoriteBooks()
  }, [loading, user]);

  async function handleListFavoriteBooks() {
     const q = query(collection(db, "books_favorite"), where("id_user", "==", user?.uid));
    const querySnapshot = await getDocs(q);

    const data = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as ISearchBooks[];
  ...
  async function handleDeleteFavoriteBook(id: string) {
    try {
      await deleteDoc(doc(db, "books_favorite", id));
       handleListFavoriteBooks()
    } catch (e) {
      toast.error("Erro ao remover livro dos favoritos");
    }
  }

  async function handleAddFavoriteBook(book: ISearchBooks) {
    try {
      await addDoc(collection(db, "books_favorite"), {
        id_user: user?.uid,
        title: book.title,
  ...
  function handleAddFavorite(book: ISearchBooks) {
    ...
    if (isFavorite) {
      handleDeleteFavoriteBook(book.id!);
  ...
  async function handleAddSaleBook(book: ISearchBooks) {
  try {
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="p-6 flex flex-col gap-6">
        <div className="w-full justify-center flex items-center gap-3 max-sm:flex-col">
          <input value={searchBook} onChange={(e) => setSearchBook(e.target.value)} type="text" placeholder="Pesquisar livros..." className="border border-gray-300 rounded py-2 px-4 w-[460px] max-sm:w-full" />
        ...
        <div className="w-full flex flex-col gap-0.5 justify-start items-center max-h-[520px] overflow-y-auto">
          {books.length > 0 && books.map((book) => (
            <div className="flex flex-col border border-gray-300 rounded p-2 w-[600px] bg-white max-sm:w-full" key={book.cover_i}>
        ...
        <div className="flex flex-col justify-center gap-4 mt-10 w-[1200px] m-auto max-sm:w-full">
                ...
                <div className="flex flex-col gap-0.5">
                  <div className="w-full flex justify-between items-center border-b border-gray-300 pb-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button className="text-blue-500 underline flex items-center gap-2 cursor-pointer" onClick={() => handleAddSaleBook(book)}>
                        <img src={amazonIcon} alt="Amazon" className="w-6 h-6" />
                        Ver na Amazon
                      </button>
